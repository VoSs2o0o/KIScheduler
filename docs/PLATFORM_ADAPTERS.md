...
Der Codex-Adapter dient als Referenz für eine strukturierte Integration. `CodexPlatform` verarbeitet die JSONL-Ereignisse von `codex exec` und gibt Ereignisse, Session-ID, Abschlussnachricht und Fehlerklasse getrennt zurück. `CodexUsageProvider` kennt keine Prozessdetails; er hängt ausschließlich von `ICodexAppServerClient` ab. Der langlebige `CodexAppServerClient` kapselt den Initialize-Handshake, parallele JSON-RPC-Anfragen und Push-Benachrichtigungen über stdio.
